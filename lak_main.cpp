...
void build_transac(vector<QueryPoint> q,int total_portions_cnt,int num_queries)
{
	...
	for(i=0;i<q.size();i++)
	{
		...
		//cout << "JJJJ " << j << " " << por_id << endl;
           	transac[por_id].push_back(i);
		}	
	}
	//cout << "TRAAANSSS " << endl;
	//cout << "transactions size = " << transac.size() << endl;
	vector<int> qps;
...
bool Mov_exp(Rectangle T, int NumOfObstacles,double vis_threshold,int no_of_portions,int num_queries, int dis_threshold)//just name, no extension needed
{
	// my own k deciding.
	int k=num_queries;
	...
	//input file
	char DATAFILE[200]="british_data.txt";
	...
	//cout << "NUmber of query points = " << (sizeof(q)/sizeof(*q)) << endl;
		...
		int min_dis=INT_MAX, max_dis=INT_MIN,avg_dis;
		Calc_query_to_target(q,T,i-1,&min_dis,&max_dis);
		avg_dis=(min_dis+max_dis)/2;
	        //cout << "no.of queies " << i-1 << "MIN DIS " << min_dis << " " << "MAX DIS " << max_dis << endl; // min = 2997, max = 88927
		//dis_threshold = 60000;
		//cout << "DISTANCE THRESHOLD " << dis_threshold << endl;
		//cout << "VISIBILITY TRSHOLD " << vis_threshold << endl;
...
int main(int argc, char* argv[])
{
	...
	//int num_queries = 500;
	//$NumOfObstacles $visthreshold $numportions $numqueries


	//obstacle size set as 100
	...
	Mov_exp(T, NumOfObstacles, vis_threshold,no_of_portions,num_queries,dis_threshold);
        return 0;
