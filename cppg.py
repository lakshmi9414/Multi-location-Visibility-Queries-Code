...
class cppg:
    def __init__(self, inputfile, outputfile, minRF, minCS, maxOR):
        ...
        #for i in sorteditems:
        	#sum = sum+i[1]
        #avg = float(float(sum)/float(l))
        #print "SUM " , sum, "len ", l, "AVG ", avg
    ...
    def cppgrec(self, pattern, length, projds):
        ...
        #print "before",  length, self.candidatecount
        self.candidatecount = self.candidatecount + len(fqs)
        #print "after",  length, self.candidatecount
        ...
        for i in fqs:
            ...
            #print "&& ",   np.sum(newostrpattern) * 1.0, np.sum(self.bitpattern[i]), ovratio
		...
		#print "for extra information", ovratio : keyword for overlap ratio;
            ...
            #print  reduce(lambda x,y: str(x) +',' + str(y), newpattern)+'\n'
            #print "overlap = ", ovratio, "cs = ", cs , "length= ", length
